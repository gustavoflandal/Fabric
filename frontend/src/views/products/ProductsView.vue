        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-32">Código</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nome</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-32">Tipo</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-24">Unidade</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-28">Estoque Mín.</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-32">Custo Padrão</th>
                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase w-24">Status</th>
                <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase w-80">Ações</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50">
                <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ product.code }}</td>
                <td class="px-3 py-3 text-sm text-gray-900">{{ product.name }}</td>
                <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{{ getTypeLabel(product.type) }}</td>
                <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{{ product.unit?.code || '-' }}</td>
                <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{{ product.minStock }}</td>
                <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-500">{{ product.standardCost ? `R$ ${product.standardCost.toFixed(2)}` : '-' }}</td>
                ...
                <td class="px-3 py-3 whitespace-nowrap text-right text-sm">
                  <div class="flex items-center justify-end space-x-3">
                    <button @click="openEditModal(product)" class="text-primary-600 hover:text-primary-900 font-medium">Editar</button>
                    <button @click="openBomManager(product)" class="text-indigo-600 hover:text-indigo-900 font-medium">BOMs</button>
                    <button @click="openRoutingManager(product)" class="text-purple-600 hover:text-purple-900 font-medium">Roteiros</button>
                    <button @click="handleToggleActive(product)" class="text-yellow-600 hover:text-yellow-900 font-medium">{{ product.active ? 'Desativar' : 'Ativar' }}</button>
                    <button @click="handleDelete(product)" class="text-red-600 hover:text-red-900 font-medium">Excluir</button>
                  </div>
                </td>
              </tr>
            </tbody>
