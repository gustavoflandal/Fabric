    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div v-if="isEditing" class="mb-6">
          <h3 class="text-lg font-medium text-gray-900 mb-4">Controle de Status</h3>
          <div class="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            />
            <label for="paused-checkbox" class="flex-1 cursor-pointer">
...
const loadPlan = async (id: string) => {
  try {
    ...
    originalStatus.value = plan.status;
    isPaused.value = plan.status === 'PAUSED';
...
const handleSubmit = async () => {
  try {
    ...
    if (isEditing.value) {
      ...
      // Gerenciar status de pausa se mudou
      const currentStatus = isPaused.value ? 'PAUSED' : originalStatus.value;
      if (isPaused.value && originalStatus.value !== 'PAUSED') {
        // Pausar plano
        await countingStore.pausePlan(planId);
      } else if (!isPaused.value && originalStatus.value === 'PAUSED') {
        // Despausar plano (reativar)
        await countingStore.activatePlan(planId);
      }
    } else {
