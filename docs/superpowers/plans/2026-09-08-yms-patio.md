...
export class YardSpotService {
  async generateBatch(areaId: string, count: number) {
    ...
    // Continua a numeração a partir da MAIOR SEQUÊNCIA já existente na área
    // (derivada dos códigos, não da contagem de linhas) — nunca reinicia do
    // zero se a área já tem vagas, para não colidir com @@unique([areaId, code])
    // mesmo depois de uma vaga do meio ter sido excluída.
